...
function chikungunya(latitude, longitude, t_start::String, t_end::String, seed_date::String)
    ...
    T_Vals = unique(WL[:,3])
    D_Vals = unique(WL[:,2])
    Wing_W       = transpose(reshape(WL[:,4], (400,400))); nodes = (T_Vals,D_Vals); sGird = interpolate(nodes,Wing_W,Gridded(Interpolations.Linear()));
    ...
    Long    = transpose(reshape(A_Long[:,4], (400,400))); nodes = (W_Vals,T_Vals); sGird = interpolate(nodes,Long,Gridded(Interpolations.Linear()));
    ...
    L_D       = transpose(reshape(L_Dur[:,4], (400,400))); nodes = (T_Vals,D_Vals); sGird = interpolate(nodes,L_D, Gridded(Interpolations.Linear()));
    ...
    nodes = (T_Vals,D_Vals); sGird = interpolate(nodes,L_S, Gridded(Interpolations.Linear()));
    ...
    function aedes_model(dy,y,h,p,t)
        ...
        # addition by Sandeep (getting R_E(t - t_E - t_L - t_P))
        RE_lag_ELP = Dia_E_L_P .* sum(repeat(q_func(TEMP_E_L_P),2) .* ylag_E_L_P[numb_A_1:numb_I_2] ./ (G_Time(TEMP_E_L_P)))  .+ on_off(t - y[numb_t_E] - y[numb_t_L] - y[numb_t_P],0)[1]
        ...
        dy[numb_D_1:numb_D_2]         = R_A
        dy[numb_RE]                   = RE_lag_ELP
        dy[numb_VC_1:numb_VC_2]       = bite(TEMP_NOW) .* bite(TEMP_NOW) .* (4*40 .*  y[numb_A_1:numb_A_2] ./ (y[numb_H_S] + y[numb_H_I] + y[numb_H_R] .+ 15000)) .* y[numb_P_EIP_1:numb_P_EIP_2] .* (0.69 ./ delta_AT(TEMP_NOW) .+ 0.0001)
        #push!(R_A_store,R_A)
    end
